...
public class SkillDrawController : MonoBehaviour
{
    ...
    [Header("궤적 유사도 판정 엄격도 (GestureMatcher 전달용)")]
    [Tooltip("정규화된 궤적(대각선=1) 기준 허용 오차. 기존 기본값은 0.5였음. 낮출수록 같은 실수(삐뚤어짐)에도 " +
             "점수가 더 크게 깎여서, 대충 그려도 threshold 근처 점수가 나오던 문제가 줄어듦.")]
    [SerializeField, Range(0.1f, 0.5f)] private float _maxAcceptableDist = 0.28f;
    [Tooltip("궤적 뒷부분 가중치 감쇠 강도. 1이면 뒷부분을 대충 그려도 점수에 거의 반영 안 됨(기존 동작). " +
             "0에 가까울수록 궤적 전체가 고르게 반영되어, 처음만 신경 쓰고 뒤는 대충 그리는 게 더 이상 안 통함.")]
    [SerializeField, Range(0f, 1f)] private float _weightFalloff = 0.4f;
    [Header("데미지 배율 (0~threshold를 min~1로, threshold~100을 1~max로 매핑)")]
    [SerializeField] private float _minDamageMultiplier = 0f;
    [SerializeField] private float _maxDamageMultiplier = 2f;
    [Tooltip("threshold 미만 구간의 매핑 곡률. 1이면 선형. 1보다 크면 애매한 점수는 min 근처에 머물다가 " +
             "threshold에 가까워질 때만 빠르게 1.0에 도달함 (대충 그린 경우 확실하게 페널티).")]
    [SerializeField, Range(0.5f, 4f)] private float _lowSegmentPower = 2f;
    [Tooltip("threshold 이상 구간의 매핑 곡률. 1이면 선형. 1보다 크면 threshold를 살짝 넘는 정도로는 배율이 " +
             "크게 안 오르고, 100점에 아주 가까워야만 최대 배율에 도달함 (정확도가 배율 차이를 크게 가름).")]
    [SerializeField, Range(0.5f, 4f)] private float _highSegmentPower = 2f;
    private Camera _cam;
    ...
    private void FinishDrawing()
    {
        ...
        {
            score = GestureMatcher.ComputeSimilarityScore(
                _allPoints,
                _guidePoints,
                resampleCount: 64,
                maxAcceptableDist: _maxAcceptableDist,
                weightFalloff: _weightFalloff);
    ...
    /// 각각 매핑하되, 단순 선형(Lerp)이 아니라 t^power 형태의 곡선으로 매핑한다.
    ///
    /// power > 1이면:
    /// - 저구간(0~threshold): 애매한 점수는 min 근처에 오래 머물다가 threshold에 가까워야 1.0에 도달
    ///   -> 대충 그려서 나온 애매한 점수가 예전처럼 threshold 근처 배율을 받는 문제를 줄임
    /// - 고구간(threshold~100): threshold를 살짝 넘는 정도로는 배율이 별로 안 오르고, 100점에 아주
    ///   가까워야만 최대 배율에 도달 -> "정확하게 그릴수록 배율 차이가 커진다"는 체감을 강화
    /// </summary>
    private float ScoreToMultiplier(float score)
    {
        if (score < _scoreThreshold)
        {
            float t = _scoreThreshold > 0f
                ...
                : 1f;
            float curvedT = Mathf.Pow(t, _lowSegmentPower);
            return Mathf.Lerp(_minDamageMultiplier, 1f, curvedT);
        }
        float tHigh = Mathf.InverseLerp(_scoreThreshold, 100f, score);
        float curvedHigh = Mathf.Pow(tHigh, _highSegmentPower);
        return Mathf.Lerp(1f, _maxDamageMultiplier, curvedHigh);
    }
