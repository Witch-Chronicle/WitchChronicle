...
/// - ResetConfirm: 스탯 초기화 확인 팝업. ResetBtn 클릭 시 열리며, Detail과 마찬가지로
///   독립 UIPanelAnimator로 열고 닫히고, StatPanel 자체가 닫히면 OnDisable()에서 같이 정리됨.
...
public class StatUIController : MonoBehaviour
{
    ...
    [Header("Panel - Reset Confirm")]
    [Tooltip("ResetBtn 클릭 시 열리는 확인 팝업입니다. Detail과 동일하게 독립 UIPanelAnimator로 관리됩니다.")]
    [SerializeField] private UIPanelAnimator _resetConfirmPanelAnimator;
    [Tooltip("확인 팝업 내부의 '초기화 진행' 버튼입니다.")]
    [SerializeField] private Button _resetConfirmBtn;
    [Tooltip("확인 팝업 내부의 '취소' 버튼입니다.")]
    [SerializeField] private Button _resetCancelBtn;
    ...
    /// <summary>
    /// 지금 스탯 초기화 확인 팝업이 열려있는지 여부. PlayerUIInputReader의 Esc 처리 분기에 사용됨.
    /// </summary>
    public bool IsResetConfirmOpen => _resetConfirmPanelAnimator != null && _resetConfirmPanelAnimator.IsOpen;
    private void Awake()
    {
        if (_closeBtn != null) _closeBtn.onClick.AddListener(ClosePanel);
        if (_resetBtn != null) _resetBtn.onClick.AddListener(OpenResetConfirmPanel);
        ...
        if (_resetConfirmBtn != null) _resetConfirmBtn.onClick.AddListener(HandleResetConfirmClicked);
        if (_resetCancelBtn != null) _resetCancelBtn.onClick.AddListener(CloseResetConfirmPanel);
        ...
        if (_resetConfirmPanelAnimator != null)
        {
            _resetConfirmPanelAnimator.SetClosedImmediate();
        }
    }
    ...
    private void OnDisable()
    {
        ...
        // C키/CloseBtn 등 어떤 경로로 닫히든 항상 여기로 들어오므로, Detail/ResetConfirm도 여기서 같이 정리.
        if (_detailPanelAnimator != null && _detailPanelAnimator.IsOpen)
        {
            _detailPanelAnimator.SetClosedImmediate();
        }
        if (_resetConfirmPanelAnimator != null && _resetConfirmPanelAnimator.IsOpen)
        {
            _resetConfirmPanelAnimator.SetClosedImmediate();
        }
    ...
    private void OpenResetConfirmPanel()
    {
        if (_resetConfirmPanelAnimator != null)
        {
            _resetConfirmPanelAnimator.Open();
        }
    }
    /// <summary>
    /// 스탯 초기화 확인 팝업만 닫습니다. (초기화는 진행하지 않음)
    /// PlayerUIInputReader의 Esc 분기와 CancelBtn 양쪽에서 사용됩니다.
    /// </summary>
    public void CloseResetConfirmPanel()
    {
        if (_resetConfirmPanelAnimator != null)
        {
            _resetConfirmPanelAnimator.Close();
        }
    }
    ...
    /// <summary>
    /// 확인 팝업의 '초기화 진행' 버튼 클릭 시 실제 리셋을 수행하고 팝업을 닫습니다.
    /// </summary>
    private void HandleResetConfirmClicked()
    {
        if (_currentStatController == null)
        {
