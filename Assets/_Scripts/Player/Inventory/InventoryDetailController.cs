...
public class InventoryDetailController : MonoBehaviour
{
    ...
    public void Show(ItemData itemData)
    {
        ...
        if (_useBtn != null)
        {
            _useBtn.gameObject.SetActive(isBook);
            // 다른 마도서를 사용하고 늦게 도착한 콜백이 interactable을 false로 남겨둔 채였을 수 있으므로,
            // 새 아이템을 보여줄 때마다 항상 true로 리셋한다.
            _useBtn.interactable = isBook;
        }
        ClearStatRows();
    ...
    private void OnClickUse()
    {
        ...
        bool started = _skillGachaOverlayController.Play(skillBook, result, () =>
        {
            ...
            // 이 콜백이 도착했을 때 이미 다른 아이템을 보고 있다면(또는 패널이 닫혔다면),
            // 지금 화면에 있는 버튼은 이 마도서(skillBook)의 것이 아니므로 절대 건드리지 않는다.
            // (예전엔 여기서 무조건 false로 덮어써서, A를 다 쓰고 B를 열었는데 B의 사용 버튼까지 꺼지는 버그가 있었음)
            ...
            _useBtn.interactable = stillOwned;
        });
