...
public sealed class PlayerUIInputReader : MonoBehaviour
{
    ...
    /// 4. StatPanel 닫기 (ResetConfirm이 열려있으면 그것만, 그 다음 Detail이 열려있으면 Detail만 먼저 닫음)
    ...
    private void HandleEscape()
    {
        ...
        {
            if (_statUIController != null && _statUIController.IsResetConfirmOpen)
            {
                _statUIController.CloseResetConfirmPanel();
                return;
            }
