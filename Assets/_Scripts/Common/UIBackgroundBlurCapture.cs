...
public sealed class UIBackgroundBlurManager : MonoBehaviour
{
    ...
    [Tooltip("Blur가 표시되는 동안 록온 표시를 임시로 숨길 컨트롤러입니다. 필드 씬이 아니면 비워둬도 됩니다.")]
    [SerializeField]
    private FieldLockOnIndicatorController _fieldLockOnIndicatorController;
    ...
    public void Show()
    {
        ...
        HideQuestList();
        HideMainHUD();
        HideDungeonPartyQueue();
        HideFieldLockOnIndicator();
        HideMinimap();
    ...
    public void Hide()
    {
        ...
        ShowQuestList();
        ShowMainHUD();
        ShowDungeonPartyQueue();
        ShowFieldLockOnIndicator();
        ShowMinimap();
    }
    /// <summary>
    ...
    /// <summary>
    /// Blur가 표시되기 전에 던전 파티 상태 표시(DungeonPartyQueueController)를 숨깁니다.
    /// Dungeon 씬이 아니면 Instance가 null이라 자동으로 아무 동작도 하지 않습니다.
    /// </summary>
    private void HideDungeonPartyQueue()
    {
        DungeonPartyQueueController.Instance?.HideContent();
    }
    private void ShowDungeonPartyQueue()
    {
        DungeonPartyQueueController.Instance?.ShowContent();
    }
    /// <summary>
    /// Blur가 표시되는 동안 필드 록온 표시를 임시로 숨깁니다.
    /// 인스펙터에 연결되지 않았다면(전투 씬 등) 아무 동작도 하지 않습니다.
    /// </summary>
    private void HideFieldLockOnIndicator()
    {
        _fieldLockOnIndicatorController?.SetExternallySuppressed(true);
    }
    private void ShowFieldLockOnIndicator()
    {
        _fieldLockOnIndicatorController?.SetExternallySuppressed(false);
    }
    /// <summary>
    /// Blur가 표시되는 동안 미니맵을 임시로 숨깁니다.
    /// Dungeon/Main 등 MinimapUIController가 없는 씬에서는 Instance가 null이라 아무 동작도 하지 않습니다.
    /// </summary>
    private void HideMinimap()
    {
        MinimapUIController.Instance?.HideContent();
    }
    private void ShowMinimap()
    {
        MinimapUIController.Instance?.ShowContent();
    }
    /// <summary>
